...
def create_empty_img():
    img = np.full((196, 256), 0.5)  # 180,240
...
def convert_event_to_img(start_index, end_index, list_event):
    ...
    img = np.full((196, 256), 0.5)  # create one channel - (row = y, column = x)
    # print 'img: ', img
    ...
    for i in range(start_index+1, end_index):
        ...
        ev_line=list_event[i]
        # print 'event line: ', ev_line

        # if type(ev_line) is list:
    ...
    # print 'img: ', img
    #print int((list_event[end_index-1].split(' '))[0])-int((list_event[start_index+1].split(' '))[0])
    # save to folder
    #print list_event[start_index+1]
    # etslne_start=list_event[start_index]
    # etsline_end = list_event[end_index-1]
    # ets_start=etslne_start[0]
    # ets_end=etsline_end[0]
    # ts=int(end_index)-int(ets_start)
    return img
